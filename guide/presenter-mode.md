...
---
```
In that case the presenter will only be available when running `kolibry` but not when running `kolibry build`.

## Remote restricted access

You can run your presentation with remote access by running `kolibry --remote`.
...
For this scenario you can provide a password for starting the server by running `kolibry --remote=your_password`.
